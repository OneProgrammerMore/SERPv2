...
const iconPaths = {
  ...
  natural:
    '<path d="M18 8h-7l-2-2H4c-1.1 0-1.99.9-1.99 2L2 20c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-2.06 11L11 14.77 8.06 19H5.12L9 13.77 5.64 9h2.94l2.42 4 2.42-4h2.94l-3.36 4.77L19 19h-3.06z"/>',
...
const resourceIconPaths = {
  police:
    '<path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 2.18l7 3.12v4.7c0 4.83-2.61 9.18-7 10.93-4.39-1.75-7-6.1-7-10.93v-4.7l7-3.12z"/>',
  firefighter:
...
const Dashboard = () => {
  ...
  const emergencies = useSelector(
    (state) => state.emergencies?.emergencies || [],
  );
  ...
  const solvedEmergencies = emergencies.filter(
    (e) => e.status === "Solved",
  ).length;
                ...
                >
                  ...
                  {filteredEmergencies.map((emergency) => (
                    <Marker
                      key={emergency.id}
                      position={[
                        emergency.location_emergency_data.latitude,
                        emergency.location_emergency_data.longitude,
                      ]}
                      icon={
                        emergencyIcons[emergency.type] || emergencyIcons.other
                      }
                    >
                      <Popup>
                      ...
                      {resources
                        .filter(
                          (resource) =>
                            ...
                            resource.location_resource_data.longitude != null,
                        )
                        .map((resource) => (
                          <Marker
                            key={resource.id}
                            position={[
                              parseFloat(
                                resource.location_resource_data.latitude,
                              ),
                              parseFloat(
                                resource.location_resource_data.longitude,
                              ),
                            ]}
                            icon={
                              resourceIcons[resource.resource_type] ||
                              resourceIcons.equipment
                            }
                          >
                            <Popup>
                              <Box sx={{ p: 1 }}>
                                ...
                                >
                                  ...
                                  <Chip
                                    size="small"
                                    label={getResourceStatusText(
                                      resource.status,
                                    )}
                                    sx={{
                                      backgroundColor: getResourceStatusColor(
                                        resource.status,
                                      ),
                                      color: "white",
                                      "& .MuiChip-label": {
